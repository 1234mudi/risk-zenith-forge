  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-amber-50 p-3 rounded-md">
        <h2 className="text-base font-medium text-amber-800 mb-1">Issues</h2>
        <p className="text-amber-700 text-xs">Record any issues or observations related to the risk being assessed.</p>
      </div>
      
      <div className="space-y-4">
        {issues.map((issue) => (
          <div key={issue.id} className="border rounded-md p-3 bg-white">
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-sm font-medium text-slate-800">Issue #{issue.id}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
              <div>
                <Label htmlFor={`issue-key-${issue.id}`} className="text-xs">Issue Key</Label>
                <Input
              ...
              <div>
                <Label htmlFor={`issue-title-${issue.id}`} className="text-xs">Title</Label>
                <Input
            ...
            <div className="mb-3">
              <Label htmlFor={`issue-description-${issue.id}`} className="text-xs">Description</Label>
              <Textarea
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor={`issue-due-date-${issue.id}`} className="text-xs">Due Date</Label>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      variant={"outline"}
                      className={cn(
                        "w-full justify-start text-left font-normal mt-1 h-8 text-sm",
              ...
              <div>
                <Label htmlFor={`issue-owner-${issue.id}`} className="text-xs">Issue Owner</Label>
                <Select
                  value={issue.owner}
                  onValueChange={(value) => handleIssueChange(issue.id, "owner", value)}
                >
                  <SelectTrigger id={`issue-owner-${issue.id}`} className="mt-1 h-8 text-sm">
      ...
      <div className="flex justify-between">
        ...
        <Button onClick={onNext} size="sm">Continue to Comments & Attachments</Button>
      </div>
    </div>
  );
};
